		...
		if(houseTileNo)
		{
		    ...
		    if(houseI)
		    {
		        ...
		        if(player->getGUID() != houseO->getOwner())
		        {
                	...
                	{
                    	player->sendTextMessage(MSG_INFO_DESCR, "Desculpe, mas voce nao tem permissao para mover item nesta casa.");
					    return NULL;
                	}
				}
			}
...
ReturnValue Game::internalMoveCreature(Creature* creature, Direction direction, uint32_t flags/* = 0*/)
{
	...
	if(ret != RET_NOERROR)
	{
		...
		{
			std::string value;
			if((player->getStorage(17000, value) && atoi(value.c_str()) >= 1) && !map->getTile(destPos)) // OrochiElf Edit
			{
				Item* newItem = Item::CreateItem(460);
				Tile* tile = IOMap::createTile(newItem, NULL, destPos.x, destPos.y, destPos.z);
				setTile(tile);
				tile->onUpdateTile();
				internalMoveCreature(player->getCreature(), (Direction)direction, FLAG_NOLIMIT);
				return RET_NOERROR;
			}
			player->sendCancelMessage(ret);
	...
	if(Player* player = creature->getPlayer())
{
	...
	if(playerLookType == 3810)
		effect = 154;
	else if(playerLookType == 3950)
		effect = 5;
	else if(playerLookType == 3811)
		effect = 105;
			else if(playerLookType == 3951)
		effect = 517;
	...
	uint16_t spriteId, int16_t fromStackpos, const Position& toPos, uint8_t count)
{
	...
	Tile* tileHoN = getMap()->getTile(toPos);
	if(tileHoN)
	{
		HouseTile* houseTileNo = tileHoN->getHouseTile();
		if(houseTileNo)
		{
		    ...
		    if(houseI)
		    {
		        ...
		        if(player->getGUID() != houseO->getOwner())
		        {
                	if(!houseO->isSubOwner(player))
                	{
                    	player->sendTextMessage(MSG_INFO_DESCR, "Desculpe, mas voce nao tem permissao para jogar item nesta casa.");
					    return NULL;
                	}
				}
			}
...
bool Game::playerSay(uint32_t playerId, uint16_t channelId, SpeakClasses type, const std::string& receiver, const std::string& text)
{
   ...
   for(ii=0; ii < int(length); ii++)
   {
      if (int(_text.find(words[ii])) > 0 || _text == words[ii])
         {
			player->sendTextMessage(MSG_STATUS_SMALL, "Respeite seus amiguinhos para não ser banido.");
             return NULL;
          break;
         }
...
bool Game::playerInviteToParty(uint32_t playerId, uint32_t invitedId)
{
	...
	if(!invitedPlayer || invitedPlayer->isRemoved() || invitedPlayer->isInviting(player) || playerId == invitedId)
		return NULL;
...
{
	Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved() || playerId == leaderId)
		return NULL;
...
{
	Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved() || !player->getParty() || player->getParty()->getLeader() != player || playerId == newLeaderId)
		return NULL;
