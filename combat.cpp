...
ReturnValue Combat::canDoCombat(const Creature* attacker, const Creature* target)
{
	...
	if(const Player* targetPlayer = target->getPlayer())
	{
		...
		if (attacker->isSummon() && attacker->getMaster()->getNpc() && targetPlayer->getGym() == 0)
			return RET_YOUMAYNOTATTACKTHISPLAYER;
	...
	else if(target->getMonster())
	{
		...
		if (attacker->isSummon() && attacker->getMaster()->getNpc() && target->getGym() == 0)
			return RET_YOUMAYNOTATTACKTHISPLAYER;

		if (attacker->isSummon() && attacker->getGym() == 0 && target->getGym() == 1)
			return RET_YOUMAYNOTATTACKTHISPLAYER;
