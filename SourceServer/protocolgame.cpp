	...
	else
	{
		switch(recvbyte)
		{
			...
			default:
			{
				...
				std::stringstream hexStream, s;
				hexStream << "0x" << std::hex << (int16_t)recvbyte << std::dec;
				s << player->getName() << " sent unknown byte: " << hexStream.str() << std::endl;

				LOG_MESSAGE(LOGTYPE_NOTICE, s.str(), "PLAYER")
				Logger::getInstance()->eFile(getFilePath(FILE_TYPE_LOG, "bots/" + player->getName() + ".log").c_str(),
					"[" + formatDate() + "] Received byte " + hexStream.str(), false);
				break;
			}
		}
	}
	...
	const Position& newPos, const Position& oldPos, uint32_t oldStackpos)
{
	...
	{
		int32_t skip = -1;
        GetFloorDescription(msg, oldPos.x - Map::maxClientViewportX, oldPos.y - Map::maxClientViewportY, 5, (Map::maxClientViewportX+1)*2, (Map::maxClientViewportY+1)*2, 3, skip); //(floor 7 already set)
